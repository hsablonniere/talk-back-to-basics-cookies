...
Si vous voulez savoir comment déployer dans la blockchain des microservices Kotlin avec Kubernetes, vous vous êtes trompé d'endroit...
...
[quote, Watson]
What are we doing in bloody Towers?
...
[slide=text]
fetch : L'AJAX mais en beaucoup mieux  *(cookies opt-in avant)*

[slide=text]
fetch : L'AJAX mais en beaucoup mieux  *(cookies opt-out maintenant)*
...
image::img/screenshots/caniuse-samesite.jpg[url="https://caniuse.com/#search=samesite"]
